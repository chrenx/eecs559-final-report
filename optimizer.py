...
class SAMSGD(SGD):
    """
    Sharpness-Aware Minimization with SGD as based optimizer
    ...
    def __init__(self,
        ...
        assert rho > 0, ValueError(f"Invalid neighborhood size: {rho}")
    ...
    def step(self, closure) -> torch.Tensor:
        ...
        for group in self.param_groups:
            ...
            grad_norm = torch.stack([g.detach().norm(2).to(device) \
                                     for g in grads]).norm(2)
            epsilon = grads 
