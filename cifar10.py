...
from optimizer import SAMSGD as _SAMSGD
...
class Optim:
    epochs: int = 200
    name: str = chika.choices("sam", "sgd", "adam")
    lr: float = 0.1
...
def _main(cfg):
    ...
    if cfg.optim.name == "sam":
        chosen_optimizer = SAM(lr=cfg.optim.lr, momentum=0.9, weight_decay=cfg.optim.weight_decay, rho=cfg.optim.rho)
    elif cfg.optim.name == "adam":
        chosen_optimizer = homura.optim.Adam(lr=cfg.optim.lr, weight_decay=cfg.optim.weight_decay)
    elif cfg.optim.name == "sgd":
        chosen_optimizer = homura.optim.SGD(lr=cfg.optim.lr, momentum=0.9, weight_decay=cfg.optim.weight_decay)

    assert chosen_optimizer is not None
...
def main(cfg: Config):
    ...
    print("currently using device: ", cfg.gpu)
    with homura.set_seed(cfg.seed):
        _main(cfg)


if __name__ == '__main__':
    # print(homura.__file__)
    # exit(0)
    main()